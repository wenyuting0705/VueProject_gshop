        ...
        <form>
          <div :class="{on:!loginWay}">
            <section class="login_message">
              ...
              <button :disabled="!isRightPhone || computeTime>0" class="get_verification" @click.prevent="sendCode"
    ...
    methods:{
     async sendCode(){
      ...
      async login(){
        ...
        if(!this.loginWay){
          if(!this.isRightPhone){
            this.sendMessage('请输入手机号')
          }else if(!/^\d{6}$/.test(code)){
            this.sendMessage('请输入验证码')
          }
          result = await reqSmslogin(phone,code)
        }else {
          if(!name){
            this.sendMessage('请输入用户名')
          }else if(!pwd){
            this.sendMessage('请输入密码')
          }else if(!captcha){
            this.sendMessage('请输入验证码')
          }
